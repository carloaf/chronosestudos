    ...
    <nav class="flex-1 px-2 py-6 space-y-1 overflow-y-auto overflow-x-hidden">
        ...
        @if (Auth::user()->isAdmin())
            ...
            <a href="{{ route('admin.users.index') }}"
               class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors
                      {{ request()->routeIs('admin.*') ? 'bg-indigo-50 text-indigo-700' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900' }}"
               :class="sidebarCollapsed ? 'justify-center' : ''"
               :title="sidebarCollapsed ? 'Administração' : ''">
                <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a4 4 0 00-3-3.87M9 20H4v-2a4 4 0 013-3.87m6-4a4 4 0 11-8 0 4 4 0 018 0zm6 0a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                <span x-show="!sidebarCollapsed" x-cloak class="whitespace-nowrap">Administração</span>
            </a>
        @endif
    </nav>
